...
# Paths Folder
output_animations = 'output/pom_animations/'
output_videos = 'output/video_with_animation/'
per_out = 'pom_output/'


# def show_pom(pom_csv, output_dir, fps):
def show_pom(per_out, output_dir, video_with_animations, segmented, fps):
    ...
    for file in output_dir.glob('*/*.csv'):
        ...
        pom_csv = f"{per_out}{filename}_{i}.csv"
        df_pom = pd.read_csv(pom_csv)
        pom_values = list(df_pom.pom.values)
        ...
        def animate(frame_num):
            ...
            y1 = pom_values[:frame_num]
            y2 = MAR_values[:frame_num]
        ...
        if video_with_animations:
            ...
            graph_video.write_videofile(f"{output_videos}{filename}/EAR-pom_{i}.mp4") 
